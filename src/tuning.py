...
# 準備
### pretrainモデルを用いる場合
get_pred_for_tuning関数を作成する必要がある
    get_pred_for_tuning要件
        引数
    ...
    get_pred_for_tuning要件
        引数
            image_source_list: 画像のパスのリスト list(str)
            confidence_threshold: confidence threshold (only detection)
            iou_threshold: iou threshold (only detection)
        返り値
            予測値 (float or int)
...
class TuningByOptuna:
    ...
    """
    def __init__(self, get_pred_for_tuning, **kwargs):
        self.get_pred_for_tuning = get_pred_for_tuning
        self.optuna_args = Args(**kwargs)

        if self.optuna_args.tuning_mode == "pretrain":
            self.set_pretrain_parameter()

            if self.optuna_args.image_type == "image":
                self.get_score = self.get_scores_for_pretrain_tuning_image
            elif self.optuna_args.image_type == "image_source_list":
                self.get_score = self.get_score_for_pretrain_tuning_image_source_list
    ...
    def compute_score(self, y_label_list, y_pred_list) -> float:
        """スコアの計算

        Args:
            y_label_list (list): ラベルのリスト
            y_pred_list (list): 予測値のリスト
    ...
    def get_scores_for_pretrain_tuning_image(self, input_width, input_height, confidence_threshold, iou_threshold ) -> float:
        """pretrainの調整のためのスコアを取得する
        画像を読み込み，予測し，スコアを計算して返す
        ...
        for image_name in glob.glob(image_source):
            ...
            if self.optuna_args.model_task == "detection":
                y_pred = self.get_pred_for_tuning(image = image, confidence_threshold = confidence_threshold, iou_threshold = iou_threshold)
            elif self.optuna_args.model_task == "crowd_counting":
                y_pred = self.get_pred_for_tuning(image = image)
        ...
        # 評価値の計算
        score = self.compute_score(y_label_list, y_pred_list)

        return score
    
    def get_score_for_pretrain_tuning_image_source_list(self, input_width: int, input_height: int, confidence_threshold: float, iou_threshold: float) -> float:
        """pretrainの調整のためのスコアを取得する
        予測する画像名をリストで渡し，予測値を取得し，スコアを計算して返す
        画像の読み込みはget_score_for_tuningに任せる
        ...
        image_source = os.path.join(self.optuna_args.source + "*")
        # 画像名のパスを含めたリスト
        image_source_list = [image_name.replace("\\", "/") for image_name in glob.glob(image_source)]
        
        # 予測
        if self.optuna_args.model_task == "detection":
            y_pred_list = self.get_pred_for_tuning(image_source_list = image_source_list,
        ...
        # csvからimage_nameのラベル取得
        y_label_list = [get_label_from_file_name(file_path = image_name, csv_path=self.optuna_args.csv_path) for image_name in image_source_list]
    ...
    def objective_pretrain(self, trial):
        ...
        for e in range(self.epoch):
            # 選択したパラメータでスコア取得
            score = self.get_score(input_width = input_width, input_height = input_height, confidence_threshold = confidence_threshold, iou_threshold = iou_threshold)
